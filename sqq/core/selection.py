...
def select_waters(atoms: list[Atom], resnames: set[str], oxygen_names: set[str], hydrogen_names: set[str]) -> list[Water]:
    ...
    for atom in atoms:
        resname_key = normalize_name(atom.resname)
        if resname_key in resname_keys:
            groups[(resname_key, atom.resid)].append(atom.index)

    waters: list[Water] = []
    for (_, resid), indices in sorted(groups.items(), key=lambda item: (item[0][1], item[0][0])):
