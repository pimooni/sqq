...
class TrackingConfig:
    """Implemented thresholds used to link cages between selected frames.

    ``gap_frame`` is the maximum number of consecutive selected frames in
    which a cage may be absent and still retain its persistent ID.  A bridged
    absence is recorded both on the destination observation and as a ``gap``
    event; it is never silent.
    """
...
class TrackCageSnapshot:
    """Small JSON-safe representation of one complete cage.

    ``water_atomids`` are stable one-based topology positions.  They do not
    use the width-limited serial stored in a GRO atom record.
    """
