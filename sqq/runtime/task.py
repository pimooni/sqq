...
from ..io.output.gro_writer import (
    ...
    *,
    ...
    try:
        ...
        if frame.time_ps is None:
            ...
            frame.time_ps = effective_frame_time_ps(
                None,
        ...
        snapshot = None
        snapshot_error = None
        if run_context.tracking_snapshots:
            try:
                snapshot = snapshot_from_frame_result(result, int(task.frame_index))
            except ValueError as exc:
                # Snapshot validation belongs to the optional temporal layer,
                # not to the otherwise valid per-frame scientific analysis.
