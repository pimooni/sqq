...
from ...io.input.gro_grouping import (
    GroGroupingResult,
    GroTopologyGroup,
...
from ...io.input.lammps import inspect_lammps_topology_mapping
from ...io.output.cleanup import (
...
from .tracking import (
    prepare_tracking_sink as _prepare_tracking_sink,
    write_analyze_tracking_outputs as _write_analyze_tracking_outputs,
)


ALL_FRAMES_FAILED_EXIT_STATUS = 1


def analyze(args: Namespace) -> int:
    """Run Analyze while exclusively owning its output root.

    Returns ``0`` when at least one frame succeeded (partial failures are
    reported in the result files) and ``ALL_FRAMES_FAILED_EXIT_STATUS`` when
    every requested frame failed, so scripts cannot mistake an empty result
...
def _completion_status(rows: Sequence[Mapping[str, Any]]) -> tuple[str, str]:
    """Return the persisted run status and error text for finished frame rows."""
    total = len(rows)
    successful = sum(
        str(row.get("status", "")).strip().casefold() == "ok" for row in rows
    )
    if total and successful == 0:
        return (
            "failed",
            f"All {total} requested frame(s) failed analysis; see run.failures in "
...
def _prepare_analyze(args: Namespace) -> _AnalyzePreparation:
    ...
    if is_cpp_mode(config.get("mode", DEFAULT_MODE)):
        # Fail once here instead of failing every frame in the worker.
        require_native()
    ...
    *,
    ...
    try:
        ...
        for key, session in render_sessions.items():
            tracking_result = tracking_results.get(key)
            bundle = session.finalize(tracking=tracking_result)
            if bundle.complete and bundle.script_path is not None:
                ...
                if tracking_result is not None:
                    tracking_results[key] = replace(
                        tracking_result,
                        source_provenance=build_tracking_source_provenance(
                            bundle,
                            tracking_result,
                        ),
                    )
    ...
    run_status, run_error = _completion_status(rows)
    root_info.update(status=run_status, error=run_error)
...
def _write_completed_reports(
    ...
    for group in grouping.groups:
        ...
        group_status, group_error = _completion_status(group_rows)
        group_info.update(status=group_status, error=group_error)
