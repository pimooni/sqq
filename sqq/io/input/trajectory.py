...
class TrajectorySelection:
    ...
    raw_frame_step: int
    # Physical time of every stored frame (``None`` when a frame has no time),
    # so planners can validate time order without re-reading the input.
    times_ps: tuple[float | None, ...] = ()
    ...
    @property
    def selected_times_ps(self) -> tuple[float | None, ...]:
        """Times of the selected frames in selection order, when known."""
        if len(self.times_ps) != self.total_frames:
            return ()
        return tuple(self.times_ps[index] for index in self.raw_indexes)


def effective_frame_time_ps(
    stored_time_ps: float | None,
    *,
    raw_frame_index: int | None,
    frame_index: int,
    input_config: Mapping[str, Any],
) -> float:
    """Return the exact physical time used by frame execution.

    A reader-provided value has precedence.  A missing value uses the same
    configured index-based fallback in Analyze and Track preflight so the
    preflight cannot approve a time sequence that execution later rejects.
    """
    if stored_time_ps is not None:
        value = float(stored_time_ps)
    else:
        time_index = raw_frame_index if raw_frame_index is not None else frame_index
        value = float(input_config.get("first_file_time_ps", 0.0)) + (
            int(time_index) * float(input_config.get("frame_time_step_ps", 1.0))
        )
