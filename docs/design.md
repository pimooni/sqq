...
SQQ-Py and SQQ-CPP keep their established three-row stage models, with unsupported or disabled stages omitted. An execution plan with exactly one task, including a directory that resolves to one file, has no `Analysis Progress` section, execution row, file row, or one-item progress bar. A multi-item process run moves scheduling details into one line such as `7 processes x 1 thread [auto; reserve 1 core]`, then shows compact aggregate stage counts, elapsed time, and an active-file table. The first table heading is `active files`, at most five rows are visible, and excess work is summarized in one overflow row. Completed/failed counts appear only in the bottom `Files` or `Frames` bar and are not repeated above it.
