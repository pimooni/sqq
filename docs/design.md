...
Graph construction implements the same `auto`, `hbond`, `oo`, and `pairs` definitions used by SQQ-Py. One shared resolver is applied before backend dispatch: `auto` selects `hbond` only when every selected water has at least two hydrogens, selects `oo` only when every selected water has none, and rejects mixed or incomplete hydrogen topology. Thus the public API and CLI cannot choose different automatic graphs. Orthorhombic minimum images and deterministic cell candidates are followed by exact distance/angle checks and stable edge sorting.

Native geometric candidate generation uses one exact cell-list traversal for periodic and non-periodic axes. Cell bins only remove impossible pairs; every retained pair is checked again with the established double-precision minimum-image distance and hydrogen-bond angle rules. The implementation does not impose a nearest-neighbor cap or approximate the cutoff. Ring adjacency and the O-O/O-H vectors reused by F3/F4 are cached within one frame, then discarded with that frame.
...
Cage growth uses the same generated face-composition constraints, Euler-compatible edge/vertex incidence, deterministic state ordering and limits, and closed-shell acceptance rules as the Python reference. Both engines always require an edge-connected, trivalent manifold shell. Optional scientific validation unwraps each ring face independently through its own cyclic edges before applying face-planarity, edge-variation, and projected-area checks; the separately unwrapped complete shell remains the source for positive volume and volume centroid. This prevents one cage-wide PBC image from distorting an individual face and matches the Python validation contract. Cage isomers use the same six-ring face-adjacency signature.
...
`pyproject.toml` uses scikit-build-core. `sqq/_version.py` is the single release-metadata source: Python imports it, package metadata reads it through the scikit-build-core regex provider, CMake receives or reads the same value, and the compiled core returns the injected `SQQ_VERSION`. CMake builds the pybind11 module `_sqq_cpp` from `sqq/core/sqq_cpp/native/src/` with a C++17 compiler and installs it inside `sqq/core/sqq_cpp/`. The handwritten source and CMake metadata are included in the source distribution but excluded from binary wheels. Native build directories, CMake/Ninja state, extension binaries, wheels, source archives, and local validation output are ignored; source `.cpp`, `.hpp`, and `CMakeLists.txt` files remain version-controlled.
...
While the pool exists, the parent sets the common BLAS/OpenMP thread environment variables to `parallel.math_thread` (default 1). A process-local reentrant lock serializes this environment mutation across concurrent API calls, and optional `threadpoolctl` also limits numerical libraries that were imported earlier. Spawned children inherit the limits before importing NumPy-backed modules. The parent environment is restored after pool shutdown.
...
Terminal headers combine the effective engine, optional special selector, and version as `SQQ: sqq-py (0.5.7)`, `sqq-cpp (0.5.7)`, `sqq-py-00 (0.5.7)`, or `sqq-cpp-99 (0.5.7)`. Per-frame information and resolved configuration retain their explicit machine-readable engine/version fields.
...
### One-page Terminal Interface

Analyze and Track own one deterministic stdout interface. The banner is followed by compact `Basic Information` and `Configuration`; `Analysis Progress` is added only when the execution plan contains more than one task. Start date, weekday, time, and UTC offset share one `Started` row; resolved input path and format share `Input`; engine/selector/version share `SQQ`; requested-to-effective graph mode and its short reason share `Graph`; ring search/report sizes and definition share `Ring`; and HALF, QUASI, and CLUSTER share `Additional search`. `Profile`, matched-file count, output-layout, `Topology`, routine `Adjustments`, default cage-report scope, quasi policy, and separate worker/backend/thread rows are not printed. An explicit cage restriction remains visible, and `Scientific validation` is presented as `Cage validation`.

SQQ-Py and SQQ-CPP keep their established three-row stage models, with unsupported or disabled stages omitted. An execution plan with exactly one task, including a directory that resolves to one file, has no `Analysis Progress` section, execution row, file row, or one-item progress bar. A multi-item parallel run moves scheduling details into one line that names the effective backend, such as `7 processes x 1 thread [auto; reserve 1 core]` or `4 worker threads x 1 thread [...]` for the thread backend, then shows compact aggregate stage counts, elapsed time, and an active-file table. The first table heading is `active files`, at most five rows are visible, and excess work is summarized in one overflow row. Completed/failed counts appear only in the bottom `Files` or `Frames` bar and are not repeated above it.

The TTY renderer rewrites one fixed live-progress region and skips identical refreshes. Terminal height controls that live-progress detail budget: very short terminals show one compact stage summary and one active file; medium terminals show up to three files; ordinary terminals show up to five. The normal successful completed page targets no more than 35 lines. There is no separate 80-column by 24-row final-results contract. Redirected/captured output contains no ANSI controls or carriage-return redraws and emits one bounded starting progress record instead of streaming repeated checkpoints; the following final-results block owns the final counts and does not repeat a terminal progress bar.

Progress measures frame science only. After the progress region closes, SQQ prints `Writing output files; please wait and do not close SQQ...` once while publishing reports, summary, render, tracking state, and resolved configuration. Warnings are normalized and deduplicated; the final page shows at most one warning preview plus a count directing the user to result files. Fatal failures remain concise error-path output.

### Final Results Page

After every selected output is safely published, an interactive TTY redraws the same page with the retained compact `Basic Information` and `Configuration` sections and replaces only the live region with `Analysis Results` or `Tracking Results`. Redirected/captured streams remain plain append-only text. The result block combines requested/analyzed/successful/failed counts on one `Frames` row and total/analysis/output-writing duration on one `Time` row. `requested` counts every selected frame plus every GRO batch input that failed the read-only pre-scan, so it is never smaller than `analyzed`, whose failed rows include those pre-scan failures. Mean time is shown only when meaningful. A fully successful run omits status, and the result path is not repeated because `Output` already remains at the top. Track source mode displays only imported-state and matching settings that affect Track.

Analyze and Track use one fixed recommendation in the terminal and summary dashboard: `Cages were identified and analyzed using SQQ.` The sentence is plain text, followed by the Publication entry, DOI, and GitHub URL as separate logical lines. The journal title is italicized in Markdown documentation. Detailed completed-feature evidence remains in persistent metadata for reproducibility but no longer lengthens the copy-ready recommendation.
...
```yaml
schema_version: 0.5.7  # managed by SQQ
...
SQQ 0.5.7 uses these responsibility boundaries:
...
- `core/tracking/{engine,snapshot,spool,targets,statistics}.py`: persistent-ID matching, compact frame state, run-private observation/event spooling, target filtering, events, lifetimes, and the shared `segment_track`/`segment_row` residence segmentation.
...
- `io/tracking/{state,network,precursor_record}.py`: streamed versioned Track state, materialized and streamed tables, target selection, single- and multi-target persistent membership conversion, directed transition networks, and the run-private precursor records.
- `io/render/{bundle,tracking}.py`: single-pass package finalization with write-time digests, target render discovery, validation, naming, and publication.
...
`sqq.load_config` accepts a YAML path, a partial mapping, or `None`, performs migration and engine-capability normalization, and returns a `ResolvedConfig` carrying an auditable `ResolutionReport`. The resolved tree is recursively copied and frozen; nested mappings, sequences, and sets cannot be changed through a retained external reference. `ResolvedConfig.to_mutable_dict()` is the explicit isolated-copy boundary used for one analysis invocation. The same SQQ-CPP capability validator is applied to CLI and API configuration, so unsupported ring definitions, sizes, searches, order parameters, output types, or thread scheduling fail or normalize identically. `sqq.read_frames` yields normalized public `Frame` values through the same supported readers used by the CLI. `sqq.analyze_frame` dispatches one `Frame` to the resolved Python or C++ backend and returns the public `FrameResult`. Configuration, input, and analysis failures use distinct exported SQQ exception types. These functions are the supported programmatic interface; workflow, runtime, and I/O modules remain implementation details.
...
7. Spatial cores are anchored only by phase-specific cages: `5^12 6^2` for sI, `5^12 6^4` for sII, and `4^3 5^6 6^3`/`5^12 6^8` for sH. Shared `5^12` cages provide spatial support without becoming mandatory anchors for competing phases. The anchors validate phase identity and later domain retention; every cage that survives the complete spatial-core checks is nevertheless included in that seed's members and internal shared-face evidence.
...
sI, sII, and sH expand independently from every strict-seed member and every validated spatial-core member. Spatial-core anchors remain a separate subset used to require phase-specific evidence when an exclusive domain is finalized. A growth candidate must:
...
After all phases collect claims independently, SQQ forms domains from cages claimed by exactly one phase. Domain edges must remain phase-compatible, and every connected domain component must contain at least one strict or spatial evidence anchor. Keeping anchors distinct from members prevents shared support cages from authenticating a phase by themselves, while seeding from all validated members avoids dropping a legitimate core cage before expansion. Same-phase regions separated by non-domain cages remain separate domains. Domain ids are deterministic within a frame and are not tracked across frames.
...
`cluster-gro` is a separate search-dependent output. Engine values `00` and `py` write it only when cluster search resolves on and YAML `output.type` selects it. Search off writes no cluster GRO output. The ordinary `gro` umbrella remains limited to ring, half-cage, quasi-cage, cage, and ice files. A later invocation aimed at an existing nonempty result uses a new numbered sibling and therefore never has to remove stale files from the earlier run.
...
One deterministic maximum-weight bipartite assignment selects the global one-to-one continuation set rather than accepting cage pairs greedily. Disconnected components of the sparse candidate graph are solved independently because no admissible edge crosses them; each local matrix retains the same global match bonus, blocked cost, dummy columns, row order, and column order. A 1x1 component still compares its candidate cost with the dummy cost instead of being accepted unconditionally. The result is mathematically identical to the dense formulation for every valid configuration while avoiding a frame-wide dense matrix. Stable snapshot sorting and assignment tie handling give newly born cages persistent IDs `t1`, `t2`, ... in deterministic order. A compatible cage retains its ID through cage-type or phase-label changes. Candidate evidence compares the current cage with the most recently observed shell of each active or dormant track, not permanently with its first shell. Progressive water exchange can therefore retain one ID only while every accepted adjacent or gap-bounding shell pair continues to satisfy the unchanged thresholds; overlap with the initial shell is not an additional requirement. Frame-local cage IDs remain stored in observation metadata for auditability.

Every accepted continuation also stores diagnostic evidence without changing the candidate gates, score, assignment, or persistent ID. The diagnostic set comprises the number of qualifying candidates sharing either endpoint (`match_candidate_count`, which includes the accepted pair itself, so an unopposed continuation reports `1`), assigned and runner-up scores, their margin, configured-threshold margins, near-threshold status, and gap evidence. `track.ambiguity_score_margin` and `track.near_threshold_tolerance` classify a match as `secure`, `ambiguous`, or `gap_bridge`; classification has no break policy and therefore cannot alter `tID` assignment. `match_diagnostic_source` records how the classification was obtained: `computed` for live tracking, `migrated_partial` when it was derived from archived evidence and archived thresholds of an older state, and empty for births and for legacy observations whose threshold-dependent diagnosis is not recoverable; `statistics/tracking_quality.csv` repeats it as `diagnostic_source`. Numeric `track.*` thresholds must be numbers: YAML `true`/`false` is rejected instead of being silently read as `1.0`/`0.0`.

`track.gap_frame` counts consecutive selected frames in which a cage may be absent. The default `0` expires an unmatched cage immediately. A positive value keeps a dormant state for at most that many selected frames; any later match records the exact gap on the observation and emits a `gap` event. Optional `track.max_gap_ps` additionally rejects a proposed bridge when the elapsed physical time between its bounding observations exceeds the limit. It is `null` by default and requires physical time on every selected frame when enabled. Selected-frame spacing may be irregular or contain equal times but must not decrease. Frame gaps use selected-frame counts, whereas `max_gap_ps`, `gap_time_ps`, duration bounds, and sampled-state integrals use the actual effective times without replacing them by a nominal interval. Snapshot indexes must remain consecutive, so an analyzed frame with no cages is represented explicitly as an empty snapshot and cannot disappear from the time axis.
...
`lifetime_distribution.csv` remains a descriptive distribution of compatibility duration samples and is not labeled censor-aware. `statistics/lifetime_survival.csv` reports two discrete-time Kaplan-Meier tables on the selected-frame grid, distinguished by the `time_basis` column. `first_absence` places a completed lifetime at its first absent selected frame (the interval upper endpoint and compatibility `lifetime_ps` convention); `last_observed` places it at its last observed frame (the interval lower endpoint). These are bracketing sensitivity estimates, not a claim that the unobserved continuous death time is exact. Right-censored tracks enter at their observed lower duration; left-censored tracks and samples without physical time are excluded and counted in `left_censored_excluded` and `time_unavailable_excluded`. Each row carries `number_at_risk`, `deaths`, `right_censored`, `survival_probability`, Greenwood variance and standard error, and bounded 95% normal-approximation `confidence_lower`/`confidence_upper`. Event IDs `e1`, `e2`, ... are assigned in chronological frame order after tracking completes, so `cage_event.csv` is stable regardless of the order in which events were detected. A track that is still within its `gap_frame` allowance when the last selected frame ends is `right_censored` without a `death` event, because its disappearance was never observed.
...
Source mode reads the Analyze engine from `sqq_config_resolved.yaml`, preserves the original `sqq-py` or `sqq-cpp` identity in terminal and newly resolved metadata, and applies C++ half/quasi capability normalization when the source engine is SQQ-CPP. An explicit `-e` with `--source` cannot change the imported engine: a different engine family (`py` for a `sqq-cpp` source or `cpp` for a `sqq-py` source) is an error, while a same-family preset (`00`/`99`) is accepted with a warning that the stored engine identity is kept. Source mode is read-only with respect to the source directory: it never writes into it, requires the render package to be exactly the four files named by the Tcl manifest, and rejects a Track result directory as a source because Track publishes target packages rather than a run-level `sqq_render/` bundle. Raw-input Track results are terminal for the same reason and store an empty `source_provenance` record.
...
`--target` accepts `all`, canonical or compact cage types, `sI`/`sII`/`sH`/boundary categories, and persistent IDs. A phase target requires SQQ-Py; SQQ-CPP phase targets fail before analysis. With raw input it automatically resolves `find_cluster` to `on`. Source mode reads imported state as saved and leaves `hydrate_cluster.enabled` untouched in its effective configuration because it cannot add phase labels retroactively; imported state must already contain them. Comma-separated mixed targets are de-duplicated and written to independent `all/`, `type_<type>/`, `phase_<phase>/`, or `cage_<tID>/` directories. Type and phase targets include the full lifecycle of any track that matched at least once, permitting residence and transition analysis outside the matching frames. Persistent-ID targets validate that the requested ID exists. A generated Track renderer passes its complete initial target-ID list to one view update, so target count does not multiply complete redraws.

The run-level `track/` directory contains state plus `cage_observation.csv`, `cage_track.csv`, `cage_event.csv`, `cage_population.csv`, `guest_residence.csv`, and `lifetime_distribution.csv`. `statistics/` adds `tracking_quality.csv`, `lifetime_survival.csv`, `cage_lineage.csv`, `guest_event.csv`, `guest_residence_lifetime.csv`, `occupancy_state_lifetime.csv`, and `occupancy_transition.csv`. `network/` always contains directed node/edge CSV tables; an optional PNG is written only when transitions exist and Matplotlib is usable. Reciprocal cage-type directions remain separate. Edge rates use accumulated directly observed source-state exposure, never first-to-last event span. Each target directory contains target-filtered copies of these tables, `track_info.md`, and `sqq_render/{sqq_track.gro,sqq_track.xtc,sqq_track.membership.tsv,sqq_track.vmd.tcl}`. Target tables are not recomputed from scratch: every per-track row of a target (`cage_track.csv`, `guest_residence.csv`, `statistics/guest_residence_lifetime.csv`, `statistics/occupancy_state_lifetime.csv`) is the run-level row with the `target` column relabelled and is obtained by filtering on track ID; observation-level tables are streamed from the selection; only frame-level and aggregate tables (population, events, lineage, transitions, survival, distribution, network) are rebuilt from the selected set. Target membership TSVs are written in batches of at most 128 destinations: ordinary requests still use one source read, while unusually large target lists use additional bounded passes instead of keeping an unbounded number of files and target maps open. Duplicate normalized destination paths are rejected. The GRO/XTC are hard-linked when the filesystem permits and copied otherwise. These coordinate files are logically immutable SQQ artifacts: unlinking or atomically replacing one path is safe, but directly editing a hard-linked file in place can also alter its sibling target packages. A target that matches no track is still published with empty tables and a selection-free render package, and the final page names it in `Warnings`; a phase target on imported frames without labels says they cannot be added retroactively, while raw Track reports that its automatically enabled cluster search produced no hydrate phase labels.

A raw-input persistent-ID target with pre-birth frames needs the history of its birth water set before the cage existed. While the single analysis pass runs, each frame is reduced to one compact precursor record (water-oxygen identities, residues, coordinates, the render atom mapping for the configured atom scope, water-water edges, and the water sets of rings, half-cages, quasi-cages, and cages) and spooled as JSON into the run-private workspace only until every requested persistent ID has first appeared. Later frames do not extend the spool. After tracking, records from frame 0 through the latest required birth frame are read and classified one at a time, so the complete prefix is never retained in memory. They classify each birth water set backwards through dispersed, connected, ring, half/quasi, and cage states, write `precursor_state.csv` and `water_history.csv`, and add precursor membership to that target's VMD package. If a record is missing, the prefix is re-analyzed through the shared runner (`Precursor Pass`) and classified as each frame completes, so both paths produce identical files without retaining the fallback record sequence. A target already present in the first selected frame has no precursor interval. Imported state lacks full pre-cage graph/ring/patch objects and writes an explicit unavailable record instead.
...
CLI Analyze and raw Track retain frame stamps plus only active/dormant cage matching state and lightweight per-track handles. Every accepted observation and emitted event is appended to a run-private SQLite spool in deterministic insertion order; final results expose repeatable read-only ordered views over that store, and target filtering stays lazy instead of copying observation/event sequences. The spool uses exact serialized Python values, is not a published result or resume checkpoint, and is deleted only after all state, reports, target packages, and the final summary have consumed it. `track_snapshots()` and `track_cages()` keep their established in-memory tuple behavior for the public programmatic API.

The bounded-memory path was exercised on a 1.417 GB, 1,001-frame LAMMPS trajectory. Analyze completed in 905.678 s with a 1.605 GB peak aggregate working set; source Track completed in 12.943 s with a 169.964 MB peak. A raw SQQ-Py Track run selecting 63 frames completed in 663.684 s with a 244.113 MB peak, stayed near 195-202 MB during analysis, and left no private Track workspace after successful publication. These measurements characterize resource use; they do not alter sampling, precision, or any scientific acceptance rule.

The ordered runtime sink consumes each compact snapshot before the executor stores a lightweight outcome with both `result` and `snapshot` cleared, so workflow bookkeeping does not recreate the sequence outside the accumulator. Per-observation records (`FrameStamp`, `TrackCageSnapshot`, `TrackFrameSnapshot`, `CageObservation`, `CageTrack`, `TrackEvent`, and the engine's internal state records) are slotted dataclasses. `track_state.json` is streamed frame by frame, track by track, and event by event in the same `indent=2` layout that `json.dumps` produces for the serialized state, and `cage_observation.csv` plus `statistics/tracking_quality.csv` are written directly from the ordered observation view. Thus neither the complete observation/event object history, complete JSON tree/text, nor per-observation row dictionaries are retained in workflow memory. The SQLite ordering keys reproduce the former stable frame/track/cage and frame/emission ordering; regression comparison covers state and every CSV byte.

Outside opt-in scientific geometry validation, both engine adapters compute the published cage center through the same engine-independent, deterministically ordered PBC-connected centroid helper. Native cage recognition and occupancy remain native; this shared final reduction removes approximately machine-epsilon summation-order differences that could otherwise perturb an exactly tied Track assignment. It changes no distance cutoff, accepted cage, coordinate precision, or matching tolerance.

A new Analyze render source stores SHA-256 identities for its four files together with atom count, fixed-topology atom identity, component signature, selected-frame/time mapping, persistent cage mapping, and Track-configuration identity; the digests, atom count, and topology identity are produced by the package writer itself. Source Track validates the Tcl manifest, nonempty files, GRO/XTC atom agreement, XTC/membership/state frame agreement, topology-stable guest atom membership, frame times, cage identities, and saved provenance before any target package is published. A discovered package reads its frame count from the `frames=N` GRO title (falling back to a membership scan for packages without it) and is otherwise re-read in full: the topology GRO and the membership TSV are hashed while they are parsed, so each is read once, and the XTC is read for its shape and its hash. Legacy states without provenance receive the same structural and semantic validation but cannot provide cryptographic source binding.

Validation covers stable and ambiguous matches, progressive water exchange at and below the acceptance boundary, irregular physical-time spacing, physical/frame gaps, PBC translation and PBC-crossing lineage, resolved and gap-unresolved type/phase changes, shared-neighbor exclusion, candidate/confirmed lineage, guest entry/exit/exchange, gap-censored occupancy residence, interval/censoring fields, directed reciprocal transitions, deterministic IDs, and v1/v2/v3/v4 state migration. Both Track paths finish through the shared final `Tracking Results` page described above.
...
Public guest membership uses one topology-stable identifier derived from the guest's minimum normalized atom index and formatted as `g########`. It does not combine `resname` with width-limited or reused residue IDs, so two distinct molecules cannot collide merely because GRO residue numbering wrapped. Python occupancy, the native adapter, render membership, and Track guest residence all use this shared identifier.
...
The renderer builds only the current frame when initialization completes. It assigns a stable name to each SQQ semantic layer, identified by family/object/color rather than frame, and creates a previously unseen layer only once. A frame trace recomputes current-frame membership and applies `mol modselect` to the existing layer, using `none` when that layer has no members; it does not run a complete-trajectory rendering pass or `mol delrep`/`mol addrep` for a frame-only redraw. The same rule applies to persistent pick-highlight layers. The former unbounded frame-by-representation atom cache is replaced by current-frame selection data, component topology metadata is retained once for frame 0, and Tcl array lookups provide constant-time target de-duplication and representation registration. The renderer snapshots actual GUI style changes and keys the reusable prototype by scope, family, cage type, and radius tier; a compatible semantic layer first created on a later frame inherits representation, color, and material without scanning all frames. Consequently, changing a DynamicBonds radius, color, or material in VMD's Graphics Representations window affects an existing layer in both earlier and later frames and supplies the prototype for compatible later-created layers. Explicit SQQ view/style changes or `sqq clear` may intentionally rebuild or reset affected SQQ layers; user-created representations are preserved. Track initialization constructs one Tcl target list and invokes one view/render update instead of repeatedly redrawing fixed-size ID chunks. Frame traces keep at most one pending idle callback, so rapid animation updates coalesce to the final frame. Re-sourcing uses the unified ownership-aware disposal path and installs one clean callback set before resetting color/selection/representation state.
...
Analyze and Track hold a nonblocking process lock on the resolved output root for the complete run. After successful publication and lock release, SQQ removes only the `.sqq.lock` file carrying that run's ownership token. Failed or interrupted runs retain the lock file for diagnosis. Atomic output-root reservation prevents concurrent invocations from selecting the same numbered sibling. Multi-GRO topology groups each receive a separate run workspace under their own result root.

Each invocation writes visualization fragments to a unique private staging directory and passes that exact path to every serial, thread, and process worker. Finalization sorts all manifests, then reads every fragment GRO exactly once in frame order: the same parsed lines are validated (record count, declared atom count, annotation column), provide the topology GRO for the first frame, yield that frame's membership rows, and yield its XTC coordinates and box, so the membership TSV and XTC are written frame by frame side by side. Temporary GRO/XTC/TSV/Tcl files are created in the destination filesystem and published with atomic replacement. The SHA-256 digests of the topology GRO, the membership TSV (after the persistent-ID rewrite), and the Tcl script are computed from the bytes as they are written; the XTC is hashed once after MDAnalysis closes it. The resulting `RenderBundle` carries those digests together with the atom count and the topology identity, so Analyze-side provenance does not re-read the published package except for one structural pass over the membership TSV. Cleanup retries transient `ENOTEMPTY`, `EBUSY`, `EACCES`, and `EPERM` errors with bounded backoff. Failure to remove staging after a successful publish is nonfatal: SQQ reports the retained directory and preserves finalized output. Scientific or validation failures remain fatal, and cleanup cannot replace the original exception.
...
Before creating run state, Analyze and Track reserve an output root. A missing or empty requested path is used directly. If that path already exists and is nonempty, SQQ atomically reserves the first available sibling named with `_001`, `_002`, and so on. It never cleans, overwrites, or mixes a new run into that pre-existing nonempty directory. The requested and resolved paths plus the automatic-rename flag are stored in resolved run metadata, and the terminal `Output` row shows the effective path. Multi-topology outputs remain owned through the one manifest at the resolved root, including their `result_A` ... `result_Z` paths.

Run-private fragment workspaces remain separately scoped by unique identifiers. Ownership-manifest paths are validated as relative and inside the resolved output root; invalid, absolute, parent-traversing, or out-of-root entries are ignored.
...
`cluster-detail` separately owns `hydrate_domain.csv` and `hydrate_cluster_detail.csv`. Explicit `cluster-detail` and `cluster-gro` require resolved cluster search on. Cluster search populates selected info and main-summary outputs without adding an unselected output type. Every engine preset requires `cluster-gro` explicitly through YAML `output.type`. Search off writes no cluster files. If no per-frame type remains selected, the pipeline removes an empty frame directory.
...
Summary publication is one recoverable transaction over the selected XLSX, main CSV, detail CSV, and cluster-detail CSV. All replacements are prepared before commit; if construction or publication fails, staged files are removed rather than exposing a mixed generation. Existing nonempty result roots, including prior `result_A`-`result_Z` layouts, remain untouched because the new invocation publishes into its separately reserved root.
