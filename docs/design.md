...
Terminal headers combine the effective engine, optional special selector, and version as `SQQ: sqq-py (0.5.4)`, `sqq-cpp (0.5.4)`, `sqq-py-00 (0.5.4)`, or `sqq-cpp-99 (0.5.4)`. Per-frame information and resolved configuration retain their explicit machine-readable engine/version fields.
...
Analyze and Track own one deterministic stdout interface. The banner is followed by compact `Basic Information`, `Configuration`, and one live `Analysis Progress` region. Start date, weekday, time, and UTC offset share one `Started` row; resolved input path and format share `Input`; engine/selector/version share `SQQ`; requested-to-effective graph mode and its short reason share `Graph`; ring search/report sizes share `Ring sizes`; and HALF, QUASI, and CLUSTER share `Additional search`. `Profile`, matched-file count, output-layout, default cage-report scope, quasi policy, and separate worker/backend/thread rows are not printed. An explicit cage restriction remains visible, and `Scientific validation` is presented as `Cage validation`.

SQQ-Py and SQQ-CPP keep their established three-row stage models, with unsupported or disabled stages omitted. An execution plan with exactly one task, including a directory that resolves to one file, has no execution row and no one-item progress bar; it shows the active stage sequence and `stage / total` timing only. A multi-item process run moves scheduling details into one line such as `7 processes x 1 thread [auto; reserve 1 core]`, then shows compact aggregate stage counts, elapsed time, and an active-file table. The first table heading is `active files`, at most five rows are visible, and excess work is summarized in one overflow row. Completed/failed counts appear only in the bottom `Files` or `Frames` bar and are not repeated above it.
...
After every selected output is safely published, an interactive TTY redraws the same page with the retained compact `Basic Information` and `Configuration` sections and replaces only the live region with `Analysis Results` or `Tracking Results`. Redirected/captured streams remain plain append-only text. The result block combines requested/analyzed/successful/failed counts on one `Frames` row and total/analysis/output-writing duration on one `Time` row. Mean time is shown only when meaningful. A fully successful run omits status, and the result path is not repeated because `Output` already remains at the top. Track source mode displays only imported-state and matching settings that affect Track.

Citation wording is generated from one explicit evidence object for features that completed and outputs that were actually published, rather than YAML requests alone. Analyze constructs this object once per topology group before summary writing and merges group evidence for the root result; the terminal results and summary dashboard consume the stored object unchanged instead of inferring features independently. The sentence uses copy-ready manuscript wording, for example `Cage types, populations, and guest occupancies were analyzed using SQQ.` Applicable clauses cover water-ring topology, half/quasi structures, cage populations and isomers, guest occupancy, hydrate phase/domain analysis, and the exact executed order parameters. VMD is mentioned only when `sqq-render` completed. Occupancy requires actual selected guest data. A one-frame Track result states only `Cage tracks were generated using SQQ.` Track type transitions and lifetimes require at least two frames; guest residence additionally requires guest IDs in the selected target observations. The same recommendation is therefore used in the terminal and summary dashboard; supported TTYs render it in bold. It always ends with the provisional publication statement and `GitHub     : https://github.com/pimooni/sqq`.

## Modules

SQQ 0.5.4 uses these responsibility boundaries:
...
`sqq_cage.vmd.tcl` resolves its neighboring files relative to the script path and validates the four-file package before scheduling renderer initialization. `source` itself does not synchronously parse the complete membership table, load the complete trajectory, or render every frame. Membership rows are parsed in event-loop slices of at most about 15 ms or 250 rows, while the GRO and XTC are opened with VMD's nonblocking `waitfor 0` path and polled until the parsed and loaded frame counts agree. The Tk Console therefore remains responsive. Help is independent of renderer readiness: `sqq`, `sqq help`, `sqq -h`, and `sqq --help` return the same guide immediately; commands that require data fail fast with `SQQ renderer is still loading`. One disposal path cancels renderer/frame/pick/label callbacks, closes the membership handle, removes SQQ traces/graphics/representations, and deletes a molecule only when the script owns it. Re-sourcing invokes that path before installing new state. Initialization failure preserves its original error after disposal and leaves help available. Successful initialization resets the default view, installs traces, and sets `ready` only as the final state transition. Default Analyze/Track view commands are queued through the renderer-ready callback. The user keeps all four files together and sources only the Tcl script:
...
When an Analyze output directory is reused, SQQ first reads `sqq_output_manifest.json`, validates every relative path against the locked output root, and removes only files and now-empty directories recorded as owned by the preceding SQQ run. The new ownership session snapshots the locked root after that cleanup and atomically publishes a replacement manifest containing files created or replaced by the current run. An unchanged pre-existing user file is never claimed. Multi-topology outputs are owned through the one manifest at the requested root, including their `result_A` ... `result_Z` paths.

For results created before the ownership manifest existed, migration cleanup is deliberately conservative: it removes only fixed SQQ paths and known summary/render locations, never arbitrary files selected from broad stem or suffix patterns. Run-private fragment workspaces remain separately scoped by unique identifiers. Invalid, absolute, parent-traversing, or out-of-root manifest entries are ignored, and unrelated files remain untouched.
