...
SQQ 0.5.3 uses these responsibility boundaries:
...
The renderer records stable names for its SQQ-owned representations and retains those representations across ordinary frame changes. A frame trace recomputes current-frame atom indexes and applies `mol modselect` to the existing layer, using `none` when that layer has no members; it does not run `mol delrep`/`mol addrep` for a frame-only redraw. The same rule applies to persistent pick-highlight layers. Consequently, changing a DynamicBonds radius, color, or material in VMD's Graphics Representations window affects that layer in both earlier and later frames. Explicit SQQ view/style changes or `sqq clear` may intentionally rebuild or reset affected SQQ layers; user-created representations are preserved. Frame traces keep at most one pending idle callback, so rapid animation updates coalesce to the final frame. Re-sourcing removes old frame and pick traces, cancels any pending callback, deletes prior SQQ graphics, and installs exactly one clean callback set before resetting color/selection/representation state.
